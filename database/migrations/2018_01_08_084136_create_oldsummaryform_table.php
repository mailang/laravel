...
class  CreateOldsummaryformTable extends Migration
{
    /**
     * Run the migrations.
     *存储汇总表的年初值
    ...
    public function up()
    {
        Schema::create('oldsummaryform', function (Blueprint $table) {
            $table->increments('oldid');
            //汇总表中的自增id
      ...
      Schema::table('oldsummaryform', function($table) {
        $table->foreign('uid')->references('id')->on('users');
        $table->foreign('id')->references('id')->on('summaryform');
      });
    }
    ...
    {
        Schema::dropIfExists('oldsummaryform');
    }
}
